...
def calculate(request):
    ...
    routes = []
    try:
	matrix, address = parse_query_string(request.META['QUERY_STRING'])
	print address
	#routes = greedy_tsp.something(tsp_input)
...
def parse_query_string(string):
    ...
    address, constraints = get_coordinates(args[0].split('=')[1]), args[1:]
    if not address:
