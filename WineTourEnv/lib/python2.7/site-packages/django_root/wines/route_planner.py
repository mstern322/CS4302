...
def sort_by_dist_from_source(dist_matrix, constraint_matrix):
	...
	ids_by_increasing_dist.append(index)
	dists_from_source[index] = max_dist + 1
    for i in range(num_nodes):
	row = []
	for j in range(num_nodes):
	    row.append(dist_matrix[ids_by_increasing_dist[i]][ids_by_increasing_dist[j]])
	new_dist_matrix.append(row)
    for i in range(num_nodes-1):
	new_constraint_matrix.append(constraint_matrix[ids_by_increasing_dist[i+1]-1])
...
def calculate_shortest_tours(source_coordinates, constraint_matrix, num_results, min_wineries, max_wineries):
    ...
    dist_matrix, constraint_matrix, new_id_to_old = sort_by_dist_from_source(dist_matrix, constraint_matrix)
    print dist_matrix
